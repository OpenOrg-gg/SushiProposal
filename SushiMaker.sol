...
contract SushiMaker is BoringOwnable {
    ...
    constructor (address _factory, address _bar, address _sushi, address _weth, address _treasury, address onsenVault) public {
       factory = IUniswapV2Factory(_factory);
       bar = _bar;
       sushi = _sushi;
       weth = _weth;
       treasury = _treasury;
       onsenVault = _onsenVault;
    }
    ...
    function _toSUSHI(address token, uint256 amountIn) internal returns(uint256 amountOut) {
        // X1 - X5: OK
        uint256 teamShare = amountIn.mul(15).div(100);
        uint256 onsenAmount = amountIn.mul(5).div(100);
        uint256 burnAmount = amountIn.mul(10).div(100);
        ...
        amountOutTeam = _swap(token, sushi, barAmount, bar);
    }
}
